...
statspath = versionpath+'STATS_C4/'
...
for fid in range(arrayid*nsites_per_id,min((arrayid+1)*nsites_per_id,len(SiteInfo))):#range(953,954):#
    ...
    theta = np.concatenate([SiteInfo[SiteInfo['clusters_4']==SiteInfo['clusters_4'].iloc[fid]].mean()[varnames[:5]].values,
...
estname = statspath+'Cluster_'+str(arrayid).zfill(3)+'.pkl'
