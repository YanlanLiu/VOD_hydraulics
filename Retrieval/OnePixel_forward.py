...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 17 00:16:59 2020
...
#for fid in range(arrayid*nsites_per_id,arrayid*nsites_per_id+1):
    ...
    VOD[IsOutlier(VOD,multiplier=2.5)] = np.nan
    ...
    counts, bin_edges1 = np.histogram(SOILM_valid, bins=bins, normed=True)
    cdf1 = np.cumsum(counts)/sum(counts)
    with open(parapath+'PARA_'+MODE+'_'+sitename+'.pkl','rb') as f: 
        PARA = pickle.load(f)
    theta = np.nanmean(PARA[1],axis=0)
    ...
    counts, bin_edges2 = np.histogram(dS1, bins=bins, normed=True)
    cdf2 = np.cumsum(counts)/sum(counts)
    dS1_matched = np.array([bin_edges2[np.abs(cdf1-cdf2[int(itm*100)]).argmin()] for itm in dS1])
# else:
...
#%%
    ...
    plt.plot(bin_edges1[1:],cdf1,label='AMSRE')
    plt.plot(bin_edges2[1:],cdf2,label='Modeled, before matching')
    counts, bin_edges3 = np.histogram(dS1_matched, bins=bins, normed=True)
    cdf3 = np.cumsum(counts)/sum(counts)
    plt.plot(bin_edges2[1:],cdf3,label='Modeled, after matching')
    counts, bin_edges4 = np.histogram(TS[3][1,:], bins=bins, normed=True)
    cdf4 = np.cumsum(counts)/sum(counts)
    # plt.plot(bin_edges4[1:],cdf4,label='Modeled, after matching')
    ...
    plt.plot(SOILM)
    plt.plot(dS1)
    plt.plot(dS1_matched)
    
    #%%
    plt.figure()
    plt.plot(dS1_matched,TS[3][1,:])
    # dS1_matched = np.array([bin_edges3[np.abs(cdf1-cdf2[int(itm*100)]).argmin()] for itm in dS1])
