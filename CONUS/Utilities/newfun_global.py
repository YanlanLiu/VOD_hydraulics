...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug  8 16:45:03 2020
...
from Utilities import LatLon, toTimestamp,savitzky_golay, dailyAvg, rm_outlier
...
nobsinaday = 8; warmup = 90; dt = 24/nobsinaday
def readCLM(inpath,sitename,timerange):
    ...
    #GLDAS_NOAH025_3H.A20141231.0300.021.nc4, GLDAS_NOAH025_3H.A20190101.2100.021.nc4
    
    # local time, adjust Greenwich time to local time based on longitutde
    tt_gldas = np.array([datetime(2014,12,31,3,0)+timedelta(hours=3*i+round(lon/15)) for i in range(len(df))])
    ...
    amidx = np.argmin(np.abs([itm.timetuple().tm_hour-6 for itm in tt_gldas[:nobsinaday]])) # SMAP passes at 6 AM
    ...
    LAI = np.interp(toTimestamp(tt_gldas),toTimestamp(tt_lai),savitzky_golay(LAI,15,1))
    ...
    discard[:warmup] = True
    dLAI = dailyAvg(LAI,nobsinaday)
    
    Forcings = (RNET,TEMP,P,VPD,Psurf,GA,LAI,VegK)
    
    smap = pd.read_csv(inpath+'SMAP/SMAP_'+sitename+'.csv').drop(columns=['Unnamed: 0'])
    augm = pd.DataFrame(np.zeros([90,2])+np.nan,columns=list(smap))
    smap = pd.concat([augm,smap],axis=0).reset_index().drop(columns=['index'])
    gleam = pd.read_csv(inpath+'GLEAM/ET_'+sitename+'.csv').drop(columns=['Unnamed: 0'])
    
    assim = pd.concat([smap,gleam],axis=1)
    ...
    return Forcings,VOD,SOILM,ET,dLAI,discard,amidx
...
para0 = [0.1,0.5,0.1]
bounds_vod = ((1e-5,10),(1e-5,10),(1e-5,10))
