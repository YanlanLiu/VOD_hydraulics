...
#%% MCMC functions
varnames = ['g1','lpx','psi50X','gpmax','C','bexp','bc','slope','sigma_et','sigma_vod','sigma_sm','loglik']

p = int(len(varnames)-1)
lowbound = np.array([0,0,0,0,0,1.5,0,0,0,0,0])
upbound = np.array([10,1,12,10,23,10,1,5,3,0.3,0.3])
...
def AMIS(lik_fun,PREFIX,samplenum,hyperpara = (0.1,0.1,20)): # AMIS sampling without parallel tempering
    ...
    logp1 = lik_fun(theta) 
    if ~np.isfinite(logp1):logp1=-9999
    ...
    for chunckid in range(0,numchunck): 
        ...
        if acc<0.01 or det<1e-100: mu,sigma,rn,acc,ii,theta,logp1 = sample_para0; print("restart...");
