...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  7 15:05:25 2020
...
from newfun_slope import readCLM, fitVOD_RMSE, AMIS
from newfun_slope import varnames,scale,dt, hour2day, hour2week
from newfun_slope import OB,CONST,CLAPP,ca
...
versionpath = parentpath + 'TroubleShooting/MC_Slope2/'; hyperpara = (0.1,0.05,20)
...
def runhh_2soil_hydro(theta):    
    g1, lpx, psi50X, gpmax,C, bexp, sbot,slope = theta[:8]
    ...
    for i in np.arange(N):
        ...
        ei= petVnumB[i]*min((s1/n)*slope,1) #**bexp#*s1/n#*soilfac#*((s1-smcwilt)/(n-smcwilt)**(1))
    ...
    s1_list[~np.isfinite(s1_list)] = np.nanmean(s1_list); s1_list[s1_list>1] = 1; s1_list[s1_list<0] = 0
