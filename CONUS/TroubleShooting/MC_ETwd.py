...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul  7 13:18:22 2020
...
versionpath = parentpath + 'TroubleShooting/MC_ETwd/'; hyperpara = (0.1,0.05,20)
...
Nobs = (sum(valid_vod)+sum(valid_et))/2
...
wwet = (wSOILM>np.nanpercentile(wSOILM,60)) & (wVPD<np.nanpercentile(wVPD,40)); wwet = wwet[valid_et]# per week
wdry = (wSOILM<np.nanpercentile(wSOILM,40)) & (wVPD>np.nanpercentile(wVPD,60)); wdry = wdry[valid_et]

ET_wd = np.nanmean(ET_valid[wwet])/np.nanmean(ET_valid[wdry])

if sum(wdry)>5 and sum(wwet)>5:
    def Gaussian_loglik(theta0):
        ...
        ET_wd_hat = np.nanmean(ET_hat[wwet])/np.nanmean(ET_hat[wdry])
        loglik_et_wd = norm.logpdf(ET_wd,ET_wd_hat,ET_wd*0.3)
        return (loglik_vod+loglik_et+loglik_et_wd)*Nobs
else:
    def Gaussian_loglik(theta0):
        theta = theta0*scale
        PSIL_hat,ET_hat,SM_hat = runhh_2soil_hydro(theta)
