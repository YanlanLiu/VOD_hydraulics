...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun 28 23:29:57 2020
...
hSOILM[~np.repeat(discard_vod,4)] = np.repeat(SM,8)
wSOILM = hour2week(hSOILM,UNIT=1)[~discard_et]
wwet = (wSOILM>np.nanpercentile(wSOILM,70)) & (wVPD<np.nanpercentile(wVPD,30)) & (valid_et) # per week
wdry = (wSOILM<np.nanpercentile(wSOILM,30)) & (wVPD>np.nanpercentile(wVPD,70)) & (valid_et)
ET_wd = np.nanmean(ET[wwet])/np.nanmean(ET[wdry])

sum(valid_et*wdry)
ET_wd_flag = 1
if min(sum(wdry),sum(valid_et))<5:
    ET_wd_flag = 0
#%%
# ========================== MCMC sampling ==============================  
...
if ET_wd_flag>0:
    def Gaussian_loglik(theta0):
        ...
        return loglik_vod+loglik_et+loglik_sm+loglik_et_wd
else:
    def Gaussian_loglik(theta0):
        theta = theta0*scale
        PSIL_hat,ET_hat,SM_hat = runhh_2soil_hydro(theta)
