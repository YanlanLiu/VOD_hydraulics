...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug  9 11:12:33 2020
...
#versionpath = parentpath + 'OSSE5/ND/'
#outpath = parentpath+'OSSE5/Test/Output/'
...
for MODE in MODE_list: 
    ...
    for count in range(nsample):
        ...
        TS = [np.concatenate([TS[ii],itm]) for ii,itm in enumerate((VOD_hat,E_hat,T_hat,np.zeros(E_hat.shape)+np.nan,PSIL_hat,dS1_matched,dS2))]
    ...
    # else:
    OBS_temporal_mean.append(np.nan); OBS_temporal_std.append(np.nan)
    ...
    r2_sm = np.apply_along_axis(nancorr,1,TS[5],SOILM_valid)**2
    er2_vod = nancorr(np.nanmedian(TS[0],axis=0),VOD_ma_valid)**2
    er2_et = nancorr(np.nanmedian(TS[1]+TS[2],axis=0),ET_valid)**2
    er2_sm = nancorr(np.nanmedian(TS[5],axis=0),SOILM_valid)**2
    acc_en = [er2_vod,er2_et,er2_sm]
    p50_pct = [trace['psi50X'].quantile(pct) for pct in [.25,.5,.75]] 
    
    wET = np.array([dailyAvg(TS[1][i,:]+TS[2][i,:],7) for i in range(TS[1].shape[0])])
    r2_et_weekly = np.apply_along_axis(nancorr,1,wET,dailyAvg(ET_valid,7))**2
    accname = statspath+'R2_'+MODE+'_'+sitename+'.pkl'
    with open(accname, 'wb') as f: 
        pickle.dump((r2_et_weekly,r2_vod,r2_et,r2_sm,p50_pct,Geweke), f)
        # pickle.dump((acc_en,r2_vod,r2_et,r2_sm,p50_pct,Geweke), f)
    
    # ======== TS stats ============
    # np.apply_along_axis()
    TS.append(np.zeros(TS[0].shape)+np.nan) # VODr_ampm, PM/AM
    TS.append(np.zeros(TS[1].shape)+np.nan) # ETr_ampm  
    
    TS_temporal_mean = [np.nanmean(itm,axis=1) for itm in TS]
    TS_temporal_std = [np.nanstd(itm,axis=1) for itm in TS]
    
    TS_temporal_mean.append(np.zeros([nsample,])+np.nan) #VODr_wd
    TS_temporal_std.append(np.zeros([nsample,])+np.nan)
    TS_temporal_mean.append(np.zeros([nsample,])+np.nan) #ETr_wd
    TS_temporal_std.append(np.zeros([nsample,])+np.nan)
    ...
    TS_temporal_mean.append(np.nan)
    TS_temporal_std.append(np.nan)
