...
if baseid==0:
    versionpath = parentpath + 'OSSE5/Low/'; noise_level = 0
elif baseid==1:
    versionpath = parentpath + 'OSSE5/Medium/'; noise_level = 1
elif baseid==2:
    versionpath = parentpath + 'OSSE5/High/'; noise_level = 2
elif baseid==3:
    versionpath = parentpath + 'OSSE5/Test/'
...
if MODE == 'VOD_ET' or ((MODE=='VOD_ET_ISO') and (res==0)): 
...
#        print(theta)
    ...
    def Gaussian_loglik(theta0):
        ...
        sigma_VOD, sigma_SM = (theta[idx_sigma_vod], theta[idx_sigma_sm])
        loglik_vod = np.nanmean(norm.logpdf(VOD_ma_valid,VOD_hat[valid_vod],sigma_VOD))
    ...
    def Gaussian_loglik(theta0):
        ...
        res_hat = nanOLS(np.column_stack([VOD_hat[::2],dLAI[::2]]), VOD_hat[1::2])
        if res_hat!=0 and np.isfinite(loglik_sm): 
            iso_hat = res_hat.params[0]
            loglik_iso = norm.logpdf(iso,iso_hat,iso*0.5)
        else: 
            loglik_iso = np.nan
