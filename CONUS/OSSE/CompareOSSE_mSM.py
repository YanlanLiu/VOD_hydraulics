...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug  9 21:07:12 2020
...
statspath3 = parentpath + 'OSSE_mSM/Medium/STATS/'
...
for mid,MODE in enumerate(MODE_list):
    ...
    for fid in set(range(Nsample)):#-set([12,13]):
        ...
        if os.path.isfile(fname):
            with open(statspath3+'R2_'+MODE+'_'+sitename+'.pkl', 'rb') as f: 
                accen, r2_vod,r2_et,r2_sm,p50_pct, Geweke = pickle.load(f)
            ACC3[:,fid] = np.array([max(r2_vod),max(r2_et),max(r2_sm),np.nanpercentile(Geweke,25)])
    ...
    # plt.plot(bin_edges[1:], cdf,color='b',ls=linetype[mid],label=MODE)
    plt.plot(ACC1[vid,:],ACC3[vid,:],'o',label=MODE)
    
plt.legend(bbox_to_anchor=(1.05,1.05))
xlim = [0,1]
plt.plot(xlim,xlim,'-k')
plt.xlabel('Moved SM');plt.ylabel('Daily ET')
plt.title('R2, VOD, medium noise')
...
statspath1 = parentpath + 'OSSE_mSM/Medium/STATS/'
statspath2 = parentpath + 'OSSE_mSM/Medium/STATS/'
