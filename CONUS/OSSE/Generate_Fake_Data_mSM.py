...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug  7 16:04:26 2020
...
from Utilities import MovAvg, nanOLS, dailyAvg,IsOutlier
import time
...
for fid in range(arrayid*nsites_per_id,(arrayid+1)*nsites_per_id):
   ...
   # Jmax = OB.koptj*OB.Hdj*np.exp(OB.Haj*(TEMP-OB.Toptj)/TEMP/CONST.R/OB.Toptj)/(OB.Hdj-OB.Haj*(1-np.exp(OB.Haj*(TEMP-OB.Toptj)/TEMP/CONST.R/OB.Toptj))) 
    ...
    for noise_level in [0,1,2]:
        ...
        if np.nanmean(SOILM_fake)>0.5:
            SOILM_fake = SOILM_fake+np.random.uniform(-0.3,0.1)
        else:
            SOILM_fake = SOILM_fake+np.random.uniform(-0.1,0.3)
