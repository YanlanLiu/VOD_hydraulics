...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 27 23:15:02 2020
...
# sns.kdeplot(TRY['P50'][TRY['PFT']==3])
pft = 3
# rvs0 = TRY['P50'][TRY['PFT']>0].values
rvs = TRY['P50'][TRY['PFT']==pft].values
shape, loc, scale = gev.fit(rvs)
# shape0, loc0, scale0 = gev.fit(rvs0)
xx = np.linspace(-12, -.01, num=200)
yy = gev.pdf(xx, shape, loc, scale)
hist, bins = np.histogram(rvs, bins=np.arange(-6,-0.01,.5), density=True)
plt.bar(bins[:-1], hist, width = .5, align='edge')
plt.plot(xx, yy, 'ro')
# plt.yscale('log')
plt.show()
print(pft,shape,loc,scale)


#%%
if SiteInfo.iloc[fid]['IGBP']==1: 
    def f_p50_prior(p50): return gev.pdf(-p50, 0.82, -5.23, 3.09)
    p50_init = 5.23
elif SiteInfo.iloc[fid]['IGBP']>9:
    def f_p50_prior(p50): return gev.pdf(-p50, 0.77, -1.87 1.27)
    p50_init = 1.85
else:
    def f_p50_prior(p50): return gev.pdf(-p50, 0.53, -2.21, 1.12)
    p50_init = 2.21
#%%
TAG = 'Test'
statspath = parentpath + 'OSSE4/'+TAG+'/STATS/'
P50 = []; G1  = []; R2  = []
